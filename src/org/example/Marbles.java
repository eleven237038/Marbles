...
public class Marbles {
    ...
    public boolean reachedDeadline(double deadlineY) {

        for(int row = 0; row < marbles.length; row++) {

            // 新增这一句
            if(marbles[row] == null) continue;

            for(int col = 0; col < marbles[row].length; col++) {

                Marble marble = marbles[row][col];

                if(marble != null) {

                    if(marble.getCenterY() >= deadlineY) {
